...
export default function NotificationSourceRow({ notificationSource }: { notificationSource: NotificationSourcesType[number] }) {
  ...
  return (
    <Table.Row
      onClick={() => router.push(`/notification-sources/${notificationSource.id}`)}
      style={{
    ...
    >
      <Table.RowHeaderCell>
        <Link
          href={`/notification-sources/${notificationSource.id}`}
          style={{
        ...
        >
          <div className="flex gap-x-4">
            ...
            <div className="flex items-center gap-x-3">
              <p className="whitespace-nowrap text-sm font-semibold leading-6 text-slate-12">{notificationSource.name}</p>
            </div>
          </div>
        </Link>
      </Table.RowHeaderCell>
