...
function TableRows({ id }: { id: string }) {
  const [notifications, _query] = api.notifications.listByNotificationSource.useSuspenseQuery({
    id,
  })
  return (
    <>
...
function NotificationsRow({ notification }: { notification: RouterOutputs["notifications"]["listByNotificationSource"][number] }) {
  return (
    <Table.Row>
      <Table.RowHeaderCell
        style={{
